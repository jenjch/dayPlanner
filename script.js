...
// Use for loop to add all divs, classes, ids, etc. (using some existing classes available in provided CSS)
  for (var i = 0; i < dailyTimes.length; i++) {
    ...
    // Generated/added text area to each row with set col and description class, and unique textAreaInput classes
...
// Created function to compare data attribute in row with moment.js to dynamically add color classes from CSS based on comparison of current moment time with each row's designated time.
  ...
  // Set up Save button click event to save to local storage. Used the Ids created for each individual button since jQuery only works on first of button of same class
  ...
  var allStorage = Object.entries(localStorage);
    // use for loop to account for all individual input fields.
   for (var i=0; i < allStorage.length; i++) {
       var pair = allStorage[i];
       var key = pair[0];
       var value = pair[1];
    // Naming the Ids in the Button row and the classes in the textarea row the same allows us to "pass" the value to output/retain input from local storage onto page.
       $("." + key).val(value);
   }
